...
async def get_stock_data(session, symbol, api_key, stats, period_info):
    ...
    if isinstance(price_data, dict) and price_data.get('results') and len(price_data['results']) > 0:
        ...
        if all(x is not None and x > 0 for x in [open_price, high_price, low_price, close_price, volume]):
            # Store with previous_ prefix for new structure
            stock_data.update({
                ...
                'previous_current_price': close_price,  # This is the close from previous day
    ...
    if isinstance(company_data, dict) and company_data.get('results'):
        ...
        # Use previous_close for calculations (this is the reference price)
...
def apply_prefilter(stock_data):
    ...
    # Check previous close price (updated to use new column name)
...
def get_fieldnames_for_prefiltered_data():
...
def get_fieldnames_for_regular_data():
...
def run_initial_data_pull(date_str=None, max_symbols=None, force_previous_day=False):
    ...
    try:
        ...
        if force_previous_day:
            from utils import get_current_cst_time
            period_info = {
        ...
        if is_prefilter_step:
            ...
            fieldnames = get_fieldnames_for_prefiltered_data()
        else:
            ...
            fieldnames = get_fieldnames_for_regular_data()
        
        if not final_results:
            raise Exception("No stocks passed pre-filtering criteria")
        
        # Write CSV with new column structure
        with open(filename, "w", newline="") as f:
            ...
            for row in final_results:
                # Only include fields that are in our fieldnames (data is already in correct format)
