...
async def get_complete_stock_data(session, symbol, api_key, stats):
    ...
    if isinstance(company_data, dict) and company_data.get('results'):
        ...
        # Also store weighted shares as backup
        weighted_shares_outstanding = result.get('weighted_shares_outstanding')
        
        # Debug logging for AAPL specifically
        if symbol == 'AAPL':
            logger.info(f"AAPL company data fields: {list(result.keys())}")
            logger.info(f"AAPL share_class_shares_outstanding: {result.get('share_class_shares_outstanding')}")
            logger.info(f"AAPL weighted_shares_outstanding: {result.get('weighted_shares_outstanding')}")
            logger.info(f"AAPL shares_outstanding: {result.get('shares_outstanding')}")
            logger.info(f"AAPL market_cap: {result.get('market_cap')}")
        ...
        elif weighted_shares_outstanding and current_price and weighted_shares_outstanding > 0:
            # Fallback to weighted shares if share_class_shares_outstanding is not available
            intraday_market_cap_millions = (weighted_shares_outstanding * current_price) / 1_000_000
            shares_outstanding = weighted_shares_outstanding  # Use as primary value
        ...
        logger.debug(f"No company data for {symbol}: {company_data}")
